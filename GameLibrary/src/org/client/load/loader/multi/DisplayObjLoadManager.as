...
package org.client.load.loader.multi
{
    ...
    public class DisplayObjLoadManager 
    {
        ...
        {
            var rslLoader:DisplayObjLoader = getFreeLoader($loadData.priority);
            if (!((rslLoader == null)))
            {
        ...
        public static function cancelLoadByUrl($url:String):void
        {
            ...
            for each (var rslLoader:DisplayObjLoader in loaderList)
            {
        ...
        private static function getFreeLoader($priority:int=0):DisplayObjLoader
        {
            var rslLoader = null;
            var lowLoader = null;
            ...
            {
                rslLoader = new DisplayObjLoader();
        ...
        private static function loadNext($rslLoader:DisplayObjLoader, $loadData:LoadData=null):void
        {
            var self = null;
            if (_isPriorityChg)
            {
        ...
        private static function initLoadEvent($rslLoader:DisplayObjLoader):void
        {
        ...
        private static function removeLoadEvent($rslLoader:DisplayObjLoader):void
        {
        ...
        {
            var rslLoader:DisplayObjLoader = (e.currentTarget as DisplayObjLoader);
            switch (e.type)
            {
                case "complete":
                    removeLoadEvent(rslLoader);
                ...
                case "ioError":
                case "securityError":
                    (trace((((("DisplayObjLoaderManager: 尝试" + rslLoader.rslLoadData.tryCount) + "次后，加载") + rslLoader.rslLoadData.url) + "失败")));
                    removeLoadEvent(rslLoader);

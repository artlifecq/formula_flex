...
public class Label extends DisplayObjectContainer implements IMeshStyle, ILayoutDisplayObject
{
	...
	private var _textOffsetX:int;
	private var _textOffsetY:int;
	...
	public override function dispose():void {
		...
		registerBitmapFonts(null);
		
		if (_image) {
			if (_enableTextBatch && DynamicTextTextureManager.instance.putSubTexture(_image.texture as SubTexture)) {
				_image.texture = null;
			} else {
				if (_image.texture) {
	...
	private function createRenderedContents():void
	{
		...
		var bitmapData:BitmapData = renderText(_text, scale, null);
		...
		if(bitmapData.width < 1 || bitmapData.height < 1 || bitmapData.width > MAX_WIDTH || bitmapData.height > MAX_HEIGHT)
		{
			...
			if(bitmapData != TextureFactory.helperBitmapData)
			{
				bitmapData.dispose();
			}
			return;
		...
		sHelperRect.setTo(_textOffsetX + _textBounds.x, _textOffsetY + _textBounds.y, _textBounds.width, _textBounds.height);
		
		var disposeBmd:Boolean = bitmapData != TextureFactory.helperBitmapData;
		
		if (_enableTextBatch) {
			texture = DynamicTextTextureManager.instance.createSubTexture(_textureKey, bitmapData, disposeBmd, sHelperRect);
		} 
		if (texture == null) {
			texture = TextureFactory.fromBitmapDataByMemoryItem(bitmapData, false, false, "bgra", disposeBmd, sHelperRect);
		}
		...
		_image.x = int(sHelperRect.x);
		_image.y = int(sHelperRect.y);
		...
		// 启用图集打包功能之后，UV会错乱掉。TODO:调研内部实现。避免该情况发生。
		if (_enableTextBatch) {
	...
	protected function measure(text : String):void
	{
		...
		if (_isHtmlText) sNativeTextField.htmlText = text;
...
//			autoScaleNativeTextField(sNativeTextField);
		...
		if(charClip)
		{
			this._textBounds.x = int(charClip.x);
			this._textBounds.y = int(charClip.y);
		}
		this._textBounds.width  = int(this._textWidth  + 0.9) + 6;
		this._textBounds.height = int(this._textHeight + 0.9) + 4;
	}
	
	private function renderText(text : String, scale:Number, fillBounds : Vector.<Rectangle>):BitmapData
	{
		...
		_textOffsetX = int(filterOffset.x);
		_textOffsetY = int(filterOffset.y + textOffsetY);
		
		sHelperRect.setTo(_textOffsetX, _textOffsetY, _textBounds.right, _textBounds.bottom);
		sHelperMatrix.setTo(1, 0, 0, 1, _textOffsetX, _textOffsetY);
		
		// finally: draw text field to bitmap data
		var helperBitmapData : BitmapData = TextureFactory.helperBitmapData;
		var useHelperBitmapData:Boolean = (sHelperRect.width <= helperBitmapData.width && sHelperRect.height <= helperBitmapData.height);
		var bitmapData:BitmapData = useHelperBitmapData ? helperBitmapData : new BitmapData(sHelperRect.width, sHelperRect.height, true, 0x0);
		if (useHelperBitmapData) {
			helperBitmapData.fillRect(sHelperRect, 0x0);
		}
		
		drawText(sNativeTextField, sHelperMatrix, bitmapData);
		
		if (fillBounds != null) {
			for (var i:int = 0; i < sNativeTextField.text.length; ++i) {
				fillBounds[fillBounds.length] = sNativeTextField.getCharBoundaries(i);
			}
		}
...
//	}
	...
	//============================动态位图字体=======================
	private var _bitmapFontsKey     : String;       
	private var _fontTextureAtlas   : TextureAtlas;
	private var _useBitmapFonts     : Boolean;
	private var _fontCharBoundaries : Vector.<Rectangle>;
	
	/**
	 * 注册位图字集，如果参数为 null，则禁用该功能。如果使用未注册字将会抛出异常。 
	 * @param chars
	 * 
	 */	
	public function registerBitmapFonts(chars : String) : void {
		...
		if (_bitmapFontsKey) {
			// 移除以前的文字
			while (_charImages && _charImages.length > 0) {
				var quad : Quad = _charImages.pop();
				if (quad != null) {
			...
			_charImages = null;
			_bitmapFontsKey = null;
			if (DynamicTextTextureManager.instance.putSubTexture(_fontTextureAtlas.texture as SubTexture)) {
				// do nothing
				_fontTextureAtlas.removeAll();
			} else {
				_fontTextureAtlas.dispose();
			}
			_fontTextureAtlas = null;
			_fontCharBoundaries.length = 0;
			_fontCharBoundaries = null;
		}
		// 禁用位图字
		if (chars == null || chars == "") {
		...
		// 注册位图字
		registerBitmapFontsToTexture(chars);
	}
	 ...
	 * 
	 */	
	private function registerBitmapFontsToTexture(chars : String) : void {
		
		_bitmapFontsKey = getTextureKey(chars);
		_fontCharBoundaries = new Vector.<Rectangle>();
		
		var bmd  : BitmapData = renderText(chars, Starling.contentScaleFactor, _fontCharBoundaries);
		sHelperRect.setTo(_textOffsetX, _textOffsetY, _hitArea.width, _hitArea.height);
		var base : IStarlingTexture = DynamicTextTextureManager.instance.createSubTexture(_bitmapFontsKey, bmd, bmd != TextureFactory.helperBitmapData, sHelperRect); 
		if (base == null) {
			base = TextureFactory.empty(bmd.width, bmd.height, false, false);
			base.root.uploadBitmapData(bmd);
			bmd.dispose();
		}
		
		_fontTextureAtlas = new TextureAtlas(base);
		
		for (var i:int = 0; i < chars.length; ++i) {
			_fontTextureAtlas.addRegion(_bitmapFontsKey + "_" + chars.charAt(i), _fontCharBoundaries[i], false);
		}
	}
	...
	private function renderBitmapFonts() : void {
		...
		var bounds : Rectangle = measureText();
		
		_charImages ||= new Vector.<Quad>();
		
		var len : int = _text.length;
		var startX : Number = bounds.x;
		var startY : int = 0;
		
		for (var i:int = 0; i < len; ++i) {
			
			var char : String = _text.charAt(i);
			var ckey : String = _bitmapFontsKey + "_" + char;
			var texture : SubTexture = _fontTextureAtlas.getTexture(ckey);
			
			if (texture == null) {
				throw new Error("Label 使用了未注册文字。");
			}
			
			var rect  : Rectangle = texture.region;
			var image : Quad = i < _charImages.length ? _charImages[i] : Pool.getQuad();
			
			// 重置UV坐标
			image.setTexCoords(0, 0, 0);
			image.setTexCoords(1, 1, 0);
			image.setTexCoords(2, 0, 1);
			image.setTexCoords(3, 1, 1);
			
			image.texture = texture;
			image.x = Math.round(startX);
			image.y = startY + rect.y;
						
			startX += rect.width;
						
			if (maxWidth > 0 && startX > maxWidth) {
				startX = bounds.x;
				startY += bounds.height + leading;
			}

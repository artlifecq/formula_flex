...
package starling.display
{
     ...
     *
     *  @see starling.textures.IStarlingTexture
     *  @see Quad
     */ 
    public class Image extends Quad
    {
        ...
        {
			/**delay call for merge "setupVertices" calls to once. @author wewell@163.com**/
			setupVerticesImmediately();
        }
		
		override public function setupVerticesImmediately():void
		{
			if (texture && _scale9Grid) setupScale9Grid();
			else if (texture && _tileGrid)
			{
				if(texture.root == texture && TextureUtils.isPowerOfTwo(texture.width) && TextureUtils.isPowerOfTwo(texture.height))
				{
					setupTileGridQuick();
				}else{
        ...
        override public function set texture(value:IStarlingTexture):void
        {
            ...
            {
                super.texture = value;
				if (value && (_scale9Grid || _tileGrid)) readjustSize();
            }
        }
        ...
        private function setupScale9Grid():void
        {
            ...
            textureBounds.setTo(0, 0, texture.width, texture.height);
        ...
        private function setupTileGrid():void
        {
            ...
            var frameWidth:Number  = _tileGrid.width  > 0 ? _tileGrid.width  : texture.width;
            var frameHeight:Number = _tileGrid.height > 0 ? _tileGrid.height : texture.height;
            ...
            var tileX:Number = 0;
            var tileY:Number = 0;
            var tileWidth:Number  = texture.width  * (frameWidth  / texture.width);
            var tileHeight:Number = texture.height * (frameHeight / texture.height);
            ...
            while (currentY < bounds.height)
            {
                ...
                while (currentX < bounds.width)
                {
					...
					TextureHelper.setTexCoords(texture,vertexData, vertexID,     texAttrName, texLeft,  texTop);
					TextureHelper.setTexCoords(texture,vertexData, vertexID + 1, texAttrName, texRight, texTop);
					TextureHelper.setTexCoords(texture,vertexData, vertexID + 2, texAttrName, texLeft,  texBottom);
					TextureHelper.setTexCoords(texture,vertexData, vertexID + 3, texAttrName, texRight, texBottom);
		...
		/**
		 * 使用uv缩放，即Gpu repeat模式,因此只一个tile,此tile大小为texture的大小;且下列等式成立
		 * this.width == texture.width * this.scaleX;
		 * this.heigt == texture.height * this.scaleY;
		 * scaleX == scaleU
		 * scaleY == sacelV
		 */		
		private function setupTileGridQuick():void
		{
			// calculate the grid of vertices simulating a repeating / tiled texture.
			// again, texture frames make this somewhat more complicated than one would think.
			var texture:IStarlingTexture = this.texture;
			...
			vertexData.setPoint(vertexID,     posAttrName, 0,  0);
			vertexData.setPoint(vertexID + 1, posAttrName, texture.width, 0);
			vertexData.setPoint(vertexID + 2, posAttrName, 0,  texture.height);
			vertexData.setPoint(vertexID + 3, posAttrName, texture.width, texture.height);
			
			TextureHelper.setTexCoords(texture,vertexData, vertexID,     texAttrName, 0,  0);
			TextureHelper.setTexCoords(texture,vertexData, vertexID + 1, texAttrName, scaleX, 0);
			TextureHelper.setTexCoords(texture,vertexData, vertexID + 2, texAttrName, 0,  scaleY);
			TextureHelper.setTexCoords(texture,vertexData, vertexID + 3, texAttrName, scaleX, scaleY);

...
CONFIG::Starling_Debug
{
		...
		public function setupVerticesImmediately():void
		{
			setupVertices();
		}
		...
		protected function setupVertices():void
        {
            ...
            var texture:IStarlingTexture = style.texture;
            ...
            if (texture)
            {
                TextureHelper.setupVertexPositions(texture,vertexData, 0, "position", _bounds);
				TextureHelper.setupTextureCoordinates(texture,vertexData, 0, texAttr);
            }
            else
            {
        ...
        {
            if (width  <= 0) width  = texture ? texture.width  : _bounds.width;
            if (height <= 0) height = texture ? texture.height : _bounds.height;
        ...
        public static function fromTexture(texture:IStarlingTexture):Quad
        {

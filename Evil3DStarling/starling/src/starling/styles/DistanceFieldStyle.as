...
class DistanceFieldEffect extends MeshEffect
{
    ...
    override protected function beforeDraw(stage3DProxy:Stage3DProxy):void
    {
        ...
        if (texture)
        {
            vertexFormat.setVertexBufferAt(stage3DProxy, 3, vertexBuffer, "basic");
            vertexFormat.setVertexBufferAt(stage3DProxy, 4, vertexBuffer, "extended");
            vertexFormat.setVertexBufferAt(stage3DProxy, 5, vertexBuffer, "outerColor");

            var pixelWidth:Number  = 1.0 / texture.root.width;
            var pixelHeight:Number = 1.0 / texture.root.height;
			...
			stage3DProxy.setProgramConstantsFromVector(Context3DProgramType.VERTEX, 5, sVector);

            sVector[0] = 0.0;
            sVector[1] = 1.0;
            sVector[2] = 2.0;

			stage3DProxy.setProgramConstantsFromVector(Context3DProgramType.VERTEX, 6, sVector);
        }
    }
    ...
    override protected function afterDraw(stage3DProxy:Stage3DProxy):void
    {
        if (texture)
        {
			stage3DProxy.setVertexBufferAt(3, null);
			stage3DProxy.setVertexBufferAt(4, null);
			stage3DProxy.setVertexBufferAt(5, null);
        }

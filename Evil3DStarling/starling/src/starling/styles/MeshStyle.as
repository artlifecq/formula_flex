...
package starling.styles
{
    ...
    public class MeshStyle extends EventDispatcher
    {
        ...
        private var _textureBase:IStarlingTexture;
        ...
        public function canBatchWith(meshStyle:MeshStyle):Boolean
        {
            ...
            {
                var newTexture:IStarlingTexture = meshStyle._texture;

                if (_texture == null && newTexture == null) return true;
                else if (_texture && newTexture)
                    return _textureBase == meshStyle._textureBase &&
						_texture.smooth == meshStyle.texture.smooth &&
                           _texture.repeat == meshStyle.texture.repeat;
                else return false;
            }
            else return false;
        }
        ...
        {
            if (_texture) return TextureHelper.getTexCoords(_texture,_vertexData, vertexID, "texCoords", out);
        ...
        {
            if (_texture) TextureHelper.setTexCoords(_texture,_vertexData, vertexID, "texCoords", u, v);
            ...
            if (value != _texture)
            {
                if (value)
                {
                    ...
                    for (i = 0; i < numVertices; ++i)
                    {
                        ...
                        TextureHelper.setTexCoords(value,_vertexData, i, "texCoords", sPoint.x, sPoint.y);
                    }
                ...
                _texture = value;
                _textureBase = value ? value.root : null;
            }
        }

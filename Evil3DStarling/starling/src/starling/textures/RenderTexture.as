...
package starling.textures
{
    ...
    public class RenderTexture extends SubTexture
    {
        ...
        private var _activeTexture:IStarlingTexture;
        private var _bufferTexture:IStarlingTexture;
                                      ...
                                      format:String="bgra")
        {
            ...
            _activeTexture = TextureFactory.empty(width, height, false, true,format);

            super(_activeTexture, new Rectangle(0, 0, width, height),true, false);

            if (persistent && useDoubleBuffering)
            {
                _bufferTexture = TextureFactory.empty(width, height, false, true,format);
                _helperImage = new Image(_bufferTexture);
				_bufferTexture.smooth = false; // solves some aliasing-issues
            }
        }
                                       ...
                                       antiAliasing:int=0):void
        {
            ...
            if (!Stage3DProxy.getInstance().recoverFromDisposal()) return;

            // switch buffers
            if (isDoubleBuffered)
            {
                var tmpTexture:IStarlingTexture = _activeTexture;

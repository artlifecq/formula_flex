		...
		public function update(selectedCfg:Q_skill_model, selectedInfo:SkillInfo):void
		{
			...
			if(riseData.q_need_items&&riseData.q_need_items.length!=0){
				...
				for(i=0;i<items.length;i++){
					...
					itemDes.push({name:item.q_name,num:obj.num,max:BackPackManager.instance.getItemCount(obj.mod)});
				}
				...
				for(i=0;i<itemDes.length;i++){
					obj=itemDes[i];
					skin.lb_daoju.htmlText+=HtmlTextUtil.getTextColor(0x25931b,obj.name)+getItemText(obj.num,obj.max)+"\n";
				}
			}
		}

		...
		private function setGetNameData(city:int,info:GuildWarCityInfo):void
		{
			...
			if(!info||!info.occupyGuildName){
				...
				if(info.attackGuildName&&(info.state==3||info.state==4)){
					...
					htmlStr+=HtmlTextUtil.getTextColor(StaticValue.GREEN_TEXT,info.occupyDay+"天\n");
					htmlStr+=HtmlTextUtil.getTextColor(StaticValue.GREEN_TEXT,"[守]");
					htmlStr+=HtmlTextUtil.getTextColor(StaticValue.YELLOW_TEXT,GSUtil.unAreaName(info.occupyGuildName)+"\n");

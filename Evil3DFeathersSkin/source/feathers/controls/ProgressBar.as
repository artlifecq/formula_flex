		...
		protected function layoutChildren():void
		{
			...
			if(_progressMode == PROGRESS_MODE_SCALE || !(this.currentFill is Sprite))
			{
				if(this._direction === Direction.VERTICAL)
				{
					...
					this.currentFill.height = Math.round(percentage * (this.actualHeight - this._paddingTop - this._paddingBottom));
					this.currentFill.x = this._paddingLeft;
					this.currentFill.y = this.actualHeight - this._paddingBottom - this.currentFill.height;
				}
				else //horizontal
				{
					this.currentFill.width = Math.round(percentage * (this.actualWidth - this._paddingLeft - this._paddingRight));
					this.currentFill.height = this.actualHeight - this._paddingTop - this._paddingBottom;
					this.currentFill.x = this._paddingLeft;
					this.currentFill.y = this._paddingTop;
				}
			}else{//_progressMode == PROGRESS_MODE_MASK
				if(this._direction === Direction.VERTICAL)
				{
					w = this.actualWidth - this._paddingLeft - this._paddingRight;
					h = Math.round(percentage * (this.actualHeight - this._paddingTop - this._paddingBottom));
					this._clipRect.readjustSize(w, h);
					this.currentFill.width = this.actualWidth - this._paddingLeft - this._paddingRight;
					this.currentFill.height = Math.round((this.actualHeight - this._paddingTop - this._paddingBottom));
					this.currentFill.x = this._paddingLeft;
					this.currentFill.y = this.actualHeight - this._paddingBottom - this.currentFill.height;
					if(this.currentFill is Sprite)
					{
						Sprite(this.currentFill).mask = _clipRect; 
						Sprite(this.currentFill).visible = w > 0 && h > 0;
					}
				}
				else //horizontal
				{
					w = Math.round( percentage * (this.actualWidth - this._paddingLeft - this._paddingRight));
					h = this.actualHeight - this._paddingTop - this._paddingBottom;
					this._clipRect.readjustSize(w, h);
					this.currentFill.width = Math.round((this.actualWidth - this._paddingLeft - this._paddingRight));
					this.currentFill.height = this.actualHeight - this._paddingTop - this._paddingBottom;
					this.currentFill.x = this._paddingLeft;
					this.currentFill.y = this._paddingTop;
					if(this.currentFill is Sprite)

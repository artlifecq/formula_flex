...
package feathers.controls
{
	 ...
	 * <listing version="3.0">
	 * var loader:ImageLoader = new ImageLoader();
	 * loader.source = TextureFactory.fromBitmap( bitmap );
	 * this.addChild( loader );</listing>
	 */
	public class ImageLoader extends FeathersControl
	{
		 ...
		 * @private
		 */
		protected var _currentTexture:IStarlingTexture;
		...
		/**
		 * The <code>IStarlingTexture</code> to display, or a URL pointing to an image
		 ...
		 *
		 * <listing version="3.0">
		 * loader.source = TextureFactory.fromBitmap( bitmap );</listing>
		 *
		...
		public function set source(value:Object):void
		{
			...
			var oldTexture:IStarlingTexture;
			//we should try to reuse the existing texture, if possible.
			if(this._isTextureOwner && value && !(value is IStarlingTexture))
			{
			...
			this._lastURL = null;
			if(this._source is IStarlingTexture)
			{
				this._isLoaded = true;
			}
			else
		 ...
		 * @private
		 */
		protected var _loadingTexture:IStarlingTexture;
		 ...
		 * @see #errorTexture
		 */
		public function get loadingTexture():IStarlingTexture
		{
		 ...
		 * @private
		 */
		public function set loadingTexture(value:IStarlingTexture):void
		{
		 ...
		 * @private
		 */
		protected var _errorTexture:IStarlingTexture;
		 ...
		 * @see #loadingTexture
		 */
		public function get errorTexture():IStarlingTexture
		{
		 ...
		 * @private
		 */
		public function set errorTexture(value:IStarlingTexture):void
		{
		...
		/**
		 * The scale factor value to pass to <code>TextureFactory.fromBitmapData()</code>
		...
		protected function commitData():void
		{
			...
			{
				this._lastURL = null;
				this._texture = IStarlingTexture(this._source);
		...
		{
			var newTexture:IStarlingTexture = this._isLoaded ? this._texture : null;
			if(!newTexture)
			{
			...
			this._originalTextureWidth = this._currentTexture.width;
			this._originalTextureHeight = this._currentTexture.height;
		...
		protected function replaceBitmapDataTexture(bitmapData:BitmapData):void
		{
			...
			if(!this._texture)
			{
				...
				this._texture = TextureFactory.empty(bitmapData.width,
					bitmapData.height, false, false,this._textureFormat);
				var sourceURL:String = this._source as String;
		...
		protected function replaceRawTextureData(rawData:ByteArray):void
		{
			...
			if(this._texture)
			{
				this._texture.root.loadData(rawData);
			}
			else
			{
				this._texture = TextureFactory.fromAtfBytes(rawData);
				var sourceURL:String = this._source as String;
				if(this._textureCache)
				{
					this._textureCache.addTexture(sourceURL, this._texture, true);
		 ...
		 * @private
		 */
		protected function createTextureOnRestore(texture:IStarlingTexture, source:String,
		...
		protected function loader_completeHandler(event:flash.events.Event):void
		{
			...
			var canReuseTexture:Boolean = this._texture &&
				this._texture.width === bitmapData.width &&
				this._texture.height === bitmapData.height &&

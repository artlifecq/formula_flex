...
package feathers.controls
{
	 ...
	 */
	public class Button extends BasicButton implements IFocusDisplayObject, ITextBaselineControl, IDragSource
	{
		/**
		 * @private
		 */
		...
		public function set htmlText(value:String):void
		{
			...
			this._label = value;
			this.defaultLabelProperties.isHTML = true;
			this.invalidate(INVALIDATION_FLAG_DATA);
		}
		...
		/**
		 *存在icon时有效,如在Check,Radio 设置此属性有效,否则的话将间接设置为horizontalAlign
		 */	
		...
		public function get textFormatCacheProperties():Object
		{
			return _textFormatCacheProperties;
		}
		
		public var labelStateCacheProperties:Object;
...
//				this.labelTextRenderer.measureText(HELPER_POINT);
			...
			if(this._iconPosition == RelativePosition.MANUAL && labelRenderer != null)
			{
				if(this.currentIcon !== null )
				{
					currentIcon.width = newWidth;
					currentIcon.height = newHeight;
				}
			}
...
//				{
					...
					}else if(textAlign == TextFormatAlign.CENTER)
					{
						this.labelTextRenderer.x =  this._paddingLeft + (this.actualWidth - this.currentIcon.width)/2;
					}else if(textAlign == TextFormatAlign.RIGHT)
					{
						this.labelTextRenderer.x = this.currentIcon.x - this.labelTextRenderer.width  - this._gap;
					}
/*				}
				else
				{
...
//				{
					...
					if(textAlign == TextFormatAlign.LEFT)
					{
						this.labelTextRenderer.x = this.currentIcon.x + this.currentIcon.width  + this._gap;
					}else if(textAlign == TextFormatAlign.CENTER)
					{
						this.labelTextRenderer.x =  this.currentIcon.x + (this.actualWidth - this.currentIcon.x  - this.labelTextRenderer.width)/2  + this._gap;
					}else if(textAlign == TextFormatAlign.RIGHT)
					{
						this.labelTextRenderer.x = this.actualWidth - this.labelTextRenderer.width  + this._gap;
					}
//				}
/*				else
				{

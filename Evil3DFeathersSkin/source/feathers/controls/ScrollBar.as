...
package feathers.controls
{
	...
	public class ScrollBar extends FeathersControl implements IDirectionalScrollBar
	{
		 ...
		 * <code>Button</code>. This factory can be used to change
		...
		protected function layoutStepButtons():void
		{
			...
			if(this._direction == Direction.VERTICAL)
			{
				decrementHeight =  !_showSliderMode ? this.decrementButton.height : 0;
				incrementHeight =  !_showSliderMode ? this.incrementButton.height : 0;
			}
			else
			{
				decrementWidth = !_showSliderMode ? this.decrementButton.width : 0;
				incrementWidth =  !_showSliderMode ? this.incrementButton.width : 0;
			}
		...
		//==========================zhuzhongmao=================================
		private var _showSliderMode:Boolean = false;
		private var decrementHeight:int;
		private var decrementWidth:int 
		private var incrementHeight:int
		private var incrementWidth:int 
		public function set showSliderMode(value:Boolean):void
		{
			_showSliderMode = value;
		}
		
		private var _thumbAutoResize:Boolean = true;
		public function set thumbAutoResize(value:Boolean):void
		{
		...
		protected function layoutThumb():void
		{
			...
			if(this._direction == Direction.VERTICAL)
			{
				contentHeight -= (decrementHeight + incrementHeight);
				var thumbMinHeight:Number = this.thumb.minHeight > 0 ? this.thumb.minHeight : this.thumbOriginalHeight;
				if(!thumbMinHeight)thumbMinHeight = 20;
				var thumbHeight:Number = Math.max(thumbMinHeight, contentHeight * adjustedPage / range) ;
				this.thumb.width = this.thumbOriginalWidth;
				if(!this._thumbAutoResize)
				{
					this.thumb.height = this.thumbOriginalHeight;
				}else if(this.thumb.maxHeight !== Number.POSITIVE_INFINITY)
				{
					this.thumb.height = Math.min(thumbHeight,  this.thumb.maxHeight);
				}else
				{
				...
				this.thumb.x = this._paddingLeft + this.thumb.paddingLeft - this._paddingRight - this.thumb.paddingRight +(this.actualWidth - this.thumb.width) / 2;
				this.thumb.y = decrementHeight + this.minimumTrack.paddingTop+this.thumb.paddingTop + this._paddingTop + Math.max(0, Math.min(trackScrollableHeight, trackScrollableHeight * (this._value - this._minimum) / range));
			}
			else //horizontal
			{
				contentWidth -= (decrementWidth + incrementWidth);
				var thumbMinWidth:Number = this.thumb.minWidth > 0 ? this.thumb.minWidth : this.thumbOriginalWidth;
				if(!thumbMinWidth)thumbMinWidth = 20;
				var thumbWidth:Number = Math.max(thumbMinWidth, contentWidth * adjustedPage / range) - this.thumb.paddingLeft - this.thumb.paddingRight;
				if(!this._thumbAutoResize)
				{
					this.thumb.width = this.thumbOriginalWidth;
				}
				else if(this.thumb.maxWidth !== Number.POSITIVE_INFINITY)
				{
					this.thumb.width = Math.min(thumbWidth, this.thumb.maxWidth);
				}else
				{
				...
				this.thumb.x = decrementWidth + this.thumb.paddingLeft + this.thumb.paddingRight + this._paddingLeft + Math.max(0, Math.min(trackScrollableWidth, trackScrollableWidth * (this._value - this._minimum) / range));
		...
		protected function layoutTrackWithMinMax():void
		{
			...
			if(this._direction === Direction.VERTICAL)
			{
				this.minimumTrack.x = this._paddingLeft - this._paddingRight;
				if(showButtons)
				{
				...
				this.maximumTrack.x = this._paddingLeft - this._paddingRight;
			...
			else //horizontal
			{
				if(showButtons)
				{
					this.minimumTrack.x = this.decrementButton.width + this._paddingLeft - this._paddingRight;
				}
				else
				{
					this.minimumTrack.x = this._paddingLeft - this._paddingRight;
				...
				this.maximumTrack.x = this.minimumTrack.x + this.minimumTrack.width+ this._paddingLeft - this._paddingRight;
		...
		public function set scrollBarVisible(value:Boolean):void
		{
			if(_scrollBarVisible != value)
			{
				...
				this.thumb.visible = this.minimumTrack.visible = value;
				this.incrementButton.visible =  this.decrementButton.visible = !_showSliderMode && value;
...
//			if(!showButtons)return;
			...
			if(this._direction == DIRECTION_VERTICAL)
			{
				...
				if(showButtons)
				{
					this.minimumTrack.y = decrementHeight + this.minimumTrack.paddingTop;
				}
				else
				{
					this.minimumTrack.y = 0;
				...
				if(showButtons)
				{
					this.minimumTrack.height = this.actualHeight - this.minimumTrack.y - incrementHeight - this.minimumTrack.paddingBottom;
				}
				else
				{
			...
			else //horizontal
			{
				if(showButtons)
				{
					this.minimumTrack.x = decrementWidth + this.minimumTrack.paddingLeft;
				}
				else
				{
					this.minimumTrack.x = this._paddingLeft - this._paddingRight;
				...
				if(showButtons)
				{
					this.minimumTrack.width = this.actualWidth - this.minimumTrack.x - incrementWidth - this.minimumTrack.paddingRight;
				}
				else
				{

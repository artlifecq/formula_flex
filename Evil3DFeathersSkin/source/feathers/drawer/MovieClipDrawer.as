package feathers.drawer
{
	...
	/**
	 * 影片剪辑位图化工具，通常不需要关心此类，直接调用MovieClipData.draw()方法即可。
	 * 注意：在AIR里，影片剪辑若不在显示列表，切换帧时会有残影的bug，转换前请先将MC都加到显示列表里。FP里没有这个问题。
	 * @author wewell@163.com
	 */
	public class MovieClipDrawer
	{
		/**
		 * 构造函数
		 */		
		public function MovieClipDrawer()
		{
		}
		
		/**
		 * 绘制一个显示对象的当前外观，存储其一帧位图信息到movieClipData内
		 */		
		ui_internal function drawDisplayObject(dp:DisplayObject,dxrData:MovieClipData,frame:int):void
		{
		 ...
		 * 绘制一个显示对象的当前外观，存储其一帧位图信息到movieClipData内
		 */		
		ui_internal function drawWithoutFilter(dp:DisplayObject,dxrData:MovieClipData,frame:int):void
		{
			var dpRect:Rectangle = dp.getBounds(dp);
		 ...
		 */		
		ui_internal function getColorRect(bitmapData:BitmapData):Rectangle
		{

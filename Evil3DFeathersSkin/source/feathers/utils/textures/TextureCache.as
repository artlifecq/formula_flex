...
package feathers.utils.textures
{
	...
	public class TextureCache
	{
		...
		{
			for each(var texture:IStarlingTexture in this._unretainedTextures)
			{
				texture.dispose();
			}
		 ...
		 * @see #hasTexture()
		 */
		public function addTexture(key:String, texture:IStarlingTexture, retainTexture:Boolean = true):void
		{
		...
		public function removeTexture(key:String, dispose:Boolean = false):void
		{
			...
			var texture:IStarlingTexture = this._unretainedTextures[key] as IStarlingTexture;
			if(texture)
			{
			...
			{
				texture = this._retainedTextures[key] as IStarlingTexture;
		 ...
		 * @see #releaseTexture()
		 */
		public function retainTexture(key:String):IStarlingTexture
		{
			if(!this._retainedTextures)
			{
			...
			if(key in this._retainedTextures)
			{
				...
				return IStarlingTexture(this._retainedTextures[key]);
			}
			
			if(!(key in this._unretainedTextures))
			{
				throw new ArgumentError("IStarlingTexture with key \"" + key + "\" cannot be retained because it has not been added to the cache.");
			}
			var texture:IStarlingTexture = IStarlingTexture(this._unretainedTextures[key]);
			this.removeUnretainedKey(key);
			this._retainedTextures[key] = texture;
			this._retainCounts[key] = 1 as int;
			return texture;
		...
		public function releaseTexture(key:String):void
		{
			...
			if(count === 0)
			{
				...
				var texture:IStarlingTexture = IStarlingTexture(this._retainedTextures[key]);

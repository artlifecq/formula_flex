...
package feathers.display
{
	 ...
	 */
	public class RepeatScale9Image extends DisplayObjectContainer implements IMeshStyle
	{
		/**
		 * @private
		 */
		...
		private function fillMesh(mesh:Image, bounds:Rectangle, texture:IStarlingTexture,clip:Rectangle=null):void
		{
			...
			if(clip != null && numTiles > 1)
			{
				if(TextureUtils.isPowerOfTwo(clip.width) && TextureUtils.isPowerOfTwo(clip.height))
				{
					var clipTexture:IStarlingTexture = GuiTheme.ins.getBitmapDataClipTexture(this._textures.styleName, clip);
					clipTexture.autoRecycleEnable = clipTexture.autoRecycleDataEnable = false;
					repeatWithGpu = clipTexture && clipTexture.root == clipTexture;
				}
			}
			...
			if(repeatWithGpu)
			{
				texture = clipTexture;
				texture.repeat = true;
				mesh = getImage(clipTexture, _imageIndex);
				_imageIndex++;
		...
		private var _imageCaches:Array;
		private var _imageIndex:int;
		private function getImage(texture:IStarlingTexture, i:int):Image
		{
			if(!_imageCaches)_imageCaches = [];
			_imageCaches[i] ||= new Image(texture);
			return _imageCaches[i];
		}

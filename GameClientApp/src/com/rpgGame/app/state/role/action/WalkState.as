package com.rpgGame.app.state.role.action
{
	...
	{
		private var _stateReference : WalkStateReference;

		public function WalkState()
		{
			super(RoleStateType.ACTION_WALK);
		}
		...
		override public function execute() : void
		{
			...
			if (_ref)
			{
				if (_ref is WalkStateReference)
				{
					_stateReference = _ref as WalkStateReference;
				}
				else
				{
					throw new Error("行走状态引用必须是WalkStateReference类型！");
				}
		...
		override public function playAnimation(role : BaseRole, render : RenderUnit3D, isFreeze : Boolean = false, time : int = -1, speedRatio : Number = 1) : void
		{
			...
			var statusType : String = RoleActionType.getActionType(RoleActionType.WALK, (_machine as RoleStateMachine).isRiding);
			switch (render.type)
			{
				...
				case RenderUnitType.MOUNT:
					render.repeat = 0;
					render.setStatus(RoleActionType.WALK, _useCrossfadeTransition ? new CrossfadeTransition(0.2) : null, time, speedRatio);
					if (isFreeze)
						render.stop(time);
					break;

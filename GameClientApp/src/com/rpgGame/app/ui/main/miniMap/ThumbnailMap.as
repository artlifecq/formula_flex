package com.rpgGame.app.ui.main.miniMap
{
	 ...
	 */
	public class ThumbnailMap extends Sprite
	{
		//-----------------------------类型
		/** 雷达小地图、主界面上**/
		...
		/**寻路的点图标**/
		private var pathIcoVect : Vector.<MapPathIco>;
		/**自己的图标**/
		private var myIco : MiniMapMainPlayerIco;
		/**格子缩放比例 **/
		...
		private var _officericons : Vector.<MapRoleIco> = new Vector.<MapRoleIco>();
		private var _familyicons : Vector.<MapRoleIco> = new Vector.<MapRoleIco>();
		private var _mapCountryMembericons : Vector.<MapRoleIco> = new Vector.<MapRoleIco>();
		private var _enemyicons : Vector.<MapRoleIco> = new Vector.<MapRoleIco>();
		private var _teammateicons : Vector.<MapRoleIco> = new Vector.<MapRoleIco>();
		private var _bornMonsterVect : Vector.<MapRoleIco> = new Vector.<MapRoleIco>;
		private var _transVect : Vector.<MapRoleIco> = new Vector.<MapRoleIco>;
		private var _npcVect : Vector.<MapRoleIco> = new Vector.<MapRoleIco>;
		private var _sceneMonsterVect : Vector.<MapRoleIco> = new Vector.<MapRoleIco>;
		private var _scenePlayerMyCountryVect : Vector.<MapRoleIco> = new Vector.<MapRoleIco>;
		private var _scenePlayerEnemyCountryVect : Vector.<MapRoleIco> = new Vector.<MapRoleIco>;
		...
		public function ThumbnailMap(rootUI : SkinUI, thumbMapType : int, miniMapWidth : int, miniMapHeight : int, scale : Number = 1)
		{
			_rootUI = rootUI;
...
//			var pos2D:Point = getChangeMapToScene(pos.x, pos.y);
		...
		public function onClearPath() : void
		{
			while (pathIcoVect.length > 0)
			{
				...
				MiniMapUtil.putMapIco(pathIcoVect.shift());
			}
		...
		private function initPanel() : void
		{
			...
			pathIcoVect = new Vector.<MapPathIco>();
			myIco = new MiniMapMainPlayerIco();
		...
		{
			var pathIco : MapPathIco = MiniMapUtil.getMapIco();
		...
		private function shiftPointFromMap() : void
		{
			...
			{
				var icon : MapPathIco = pathIcoVect[0];
...
//		 * @author 卢国征  2015-4-28
//		 */
//		private function creatEmptyPoint(beforePoint : Vector3D, lastPoint : Vector3D) : void
//		{
//			if (beforePoint == null || lastPoint == null)
//				return;
//
//			var dx : Number = lastPoint.x - beforePoint.x;
//			var dy : Number = lastPoint.z - beforePoint.z;
...
//			var sin : Number = Math.sin(radians);
//
//			var pathIco : MapPathIco;
//			var len : int = Math.ceil(d / PATH_POINT_D);
//			for (var i : int = 0; i < len; i++)
//			{
//				pathIco = MiniMapUtil.getMapIco();
//				pathIco.updatePos(getChangeSceneToMap(new Vector3D(beforePoint.x + cos * PATH_POINT_D * i, 0, beforePoint.z + sin * PATH_POINT_D * i)));
//
//				pathIcoVect.push(pathIco);
//				pathSpr.addChild(pathIco);
//			}
//		}
		...
		private function onMapTouch(e : TouchEvent) : void
		{
			...
			var pos2D : Point
			var touch : Touch;
			/*touch = e.getTouch(baseSpr, TouchPhase.HOVER);
			if (touch != null)
			{
//				pos = touch.getLocation(baseSpr);
//				pos2D = getChangeMapToScene(pos.x, pos.y);
//				curPosition[0] = Math.round(pos2D.x / 100);
//				curPosition[1] = Math.round(pos2D.y / 100);
			}*/
			...
			if (touch != null)
			{
				...
				if (touch.target is MapRoleIco)
				{
					baseRoleData = (touch.target as MapRoleIco).data;
				}
		...
		private function getChangeSceneToMap(position : Vector3D) : Point
		{
			...
			var posy : int = (absMaxHeight - position.z) * mapData.miniScaleY * this.scaleMapY;
			var tilt : Number = MathUtil.angleToRadian(mapData.miniCorrectTilt);
			var pan : Number = MathUtil.angleToRadian(mapData.miniCorrectPan);
			var roll : Number = MathUtil.angleToRadian(mapData.miniCorrectRoll);
		...
		private function clearBornMonster() : void
		{
			...
			{
				MiniMapUtil.putMapRoleIco(_bornMonsterVect[i]);
		...
		private function clearTrans() : void
		{
			...
			{
				MiniMapUtil.putMapRoleIco(_transVect[i]);
		...
		private function clearNpcs() : void
		{
			...
			{
				MiniMapUtil.putMapRoleIco(_npcVect[i]);
				_npcVect.splice(i, 1);
			}
		}
		...
		{
			var ico : MapRoleIco = _moveIcoMap.remove(key);
			if (ico == null)
				return;
			var index : int;
			switch (ico.type)
		...
		private function updateMoveIco(sceneRole : SceneRole) : void
		{
			...
			{
				var ico : MapRoleIco = _moveIcoMap.getValue((sceneRole.data as RoleData).type + "_" + (sceneRole.data as RoleData).id);
				if (ico == null)
				{
					ico = MiniMapUtil.getMapRoleIco();
		...
		private function updateIcos(type : int, datas : Vector.<BaseEntityData>, icons : Vector.<MapRoleIco>) : void
		{
			var pos3d : Vector3D = new Vector3D();
			var ruledOutIcons : Vector.<MapRoleIco> = new Vector.<MapRoleIco>();
		 ...
		 * @param icons
		 */
		private function clearMapIcons(clearIcons : Vector.<MapRoleIco>, ruledOutIcons : Vector.<MapRoleIco> = null) : void
		{
			for (var i : int = clearIcons.length - 1; i >= 0; i--)
			{
				var ico : MapRoleIco = clearIcons[i];
				if (ruledOutIcons)
				{
		...
		private function disposeThumbnai() : void
		{

package com.editor.data
{
	...
	public class AvatarInfo
	{
		...
		private var _rpd_weapon_effects : Vector.<RenderParamData3D>;
		...
		private var _rpd_body_effects : Vector.<RenderParamData3D>;
		...
		public function setBodyResID(body : String, animat : String) : void
		{
			...
			if (_bodyResID)
			{
				_rpd_body = new RenderParamData3D(RenderUnitID.BODY, RenderUnitType.BODY, GlobalConfig.getAvatarFilePath(_bodyResID));
				if (_bodyAnimatResID)
		...
		public function set hairResID(value : String) : void
		{
			...
			if (_hairResID)
			{
				_rpd_hair = new RenderParamData3D(RenderUnitID.HAIR, RenderUnitType.HAIR, GlobalConfig.getAvatarFilePath(_hairResID));
		...
		public function setWeaponEffectResIDs(resIds : Array, bindBones : Array) : void
		{
			...
			if (_weaponEffectResIDs)
			{
				...
				{
					_rpd_weapon_effects = new Vector.<RenderParamData3D>();
				}
				...
				for (var i : int = 0; i < len; i++)
				{
					...
					if (resId)
					{
						var rpd_effect : RenderParamData3D = new RenderParamData3D(i + 1, RenderUnitType.WEAPON_EFFECTS, GlobalConfig.getEffectFilePath(resId));
		...
		public function set weaponResID(value : String) : void
		{
			...
			if (_weaponResID)
			{
				_rpd_weapon = new RenderParamData3D(RenderUnitID.WEAPON, RenderUnitType.WEAPON, GlobalConfig.getAvatarFilePath(_weaponResID));
		...
		public function set deputyWeaponResID(value : String) : void
		{
			...
			{
				_rpd_deputy_weapon = new RenderParamData3D(RenderUnitID.DEPUTY_WEAPON, RenderUnitType.DEPUTY_WEAPON, GlobalConfig.getAvatarFilePath(_deputyWeaponResID));
		...
		public function setMountResID(value : String, animat : String) : void
		{
			...
			if (_mountResID)
			{
				_rpd_mount = new RenderParamData3D(RenderUnitID.MOUNT, RenderUnitType.MOUNT, GlobalConfig.getAvatarFilePath(_mountResID));
		...
		public function set bodyMethodTypeEffectResID(value : String) : void
		{
			...
			{
				_rpd_body_method_type_effect = new RenderParamData3D(RenderUnitID.BODY_METHOD_TYPE_EFFECT, RenderUnitType.BODY_METHOD_TYPE_EFFECT, GlobalConfig.getEffectFilePath(_bodyMethodTypeEffectResID));
		...
		public function set effectResID(value : String) : void
		{
			...
			if (_effectResID)
			{
				_rpd_effect = new RenderParamData3D(RenderUnitID.EFFECT, RenderUnitType.EFFECT, GlobalConfig.getEffectFilePath(_effectResID));
		...
		public function setBodyEffectResIDs(resIds : Array, bindBones : Array) : void
		{
			...
			if (_bodyEffectResIDs)
			{
				...
				{
					_rpd_body_effects = new Vector.<RenderParamData3D>();
				}
				...
				for (var i : int = 0; i < len; i++)
				{
					...
					if (resId)
					{
						var rpd_effect : RenderParamData3D = new RenderParamData3D(i + 1, RenderUnitType.BODY_EFFECTS, GlobalConfig.getEffectFilePath(resId));
						rpd_effect.mouseEnable = true;
						rpd_effect.clearSameType = false;
		...
		/**身体特效*/
		public function get rpd_body_effects() : Vector.<RenderParamData3D>
		{
			return _rpd_body_effects;
		}

		/**武器特效*/
		public function get rpd_weapon_effects() : Vector.<RenderParamData3D>
		{

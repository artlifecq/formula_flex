package com.editor.scene.animator
{
	...
	public class CommonTrajectoryAnimator implements IRenderAnimator
	{
		...
		public function CommonTrajectoryAnimator(spellInfo : ReleaseSpellInfo, targetPos : Vector3D, targetRole : SceneRole, totalTime : int, speed : int, isTrackTarget : Boolean, matchTerrain : Boolean, isFlyCross : Boolean, isAdaptiveTargetHeight : Boolean, moveDelay : int, playDelay : int, releaseDelayTime : int, throwHeight : int, throwWeightRatio : int)
		{
			...
			if (!_polygonMaterial)
			{
				var shapeBmd : BitmapData = new BitmapData(256, 256, true, 0xff0000ff);
		...
		public function setOwner(owner : BaseObj3D) : void
		{
			...
			_flagPolygon = new Mesh(new SphereGeometry(30, 8, 8, true, false, false), _polygonMaterial);
			_renderSet.graphicDis.addChild(_flagPolygon);
		...
		{
			SceneManager.getInstance().removeSceneObjFromMainScene(_renderSet);
		}
				...
				else
				{
					...
					if (hitTime > 0)
					{
						...
						else
						{
							...
							if (_throwHeight > 0)
							{
								scene = SceneManager.getInstance().mainScene;
			...
			if (_renderSet && _renderSet.usable)
			{
				...
				else
				{
					...
					if (_renderUnit)
					{
						...
						if (effectTabel)
						{
							var rtnDatas : Array = ConfigData.getCfgByFieldValue(effectTabel.desc, "res", _renderUnit.renderParamData.sourceName);
							effectCfg = rtnDatas.length > 0 ? rtnDatas[0] as EffectAnimationConfig : null;
						}
						if (effectCfg && effectCfg.mDissipateTime > 0)
						{
							var childNames : Array = effectCfg.mCollideHideNodes.split(";");
							for each (var childName : String in childNames)
							{
								setRenderChildVisible(childName);
							}
							TweenLite.delayedCall(effectCfg.mDissipateTime * 0.001, onRemoveRender);
						}
						else
						{
							onRemoveRender();
		...
		public function dispose() : void
		{
			...
			if (_flagPolygon)
			{
				if (_flagPolygon.parent)
					_flagPolygon.parent.removeChild(_flagPolygon);
				_flagPolygon.dispose();
				_flagPolygon = null;
			}
		}
	}

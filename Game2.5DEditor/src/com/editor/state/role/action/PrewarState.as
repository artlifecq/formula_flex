	...
	public class PrewarState extends ActionState
	{
		...
		override public function playAnimation(role : BaseRole, render : RenderUnit3D, isFreeze : Boolean = false, time : int = -1, speedRatio : Number = 1) : void
		{
			...
			switch (render.type)
			{
				...
				case RenderUnitType.MOUNT:
					render.repeat = 0;
					render.setStatus(RoleActionType.PREWAR, new CrossfadeTransition(0.2), time);
					break;
		...
		override public function enterPass(prevState : IState, force : Boolean = false) : Boolean
		{
			if (prevState)
			{
				...
				else if (prevState.type == RoleStateType.ACTION_TRAIL)
				{
					return false;
				}
				else if (prevState.type == RoleStateType.ACTION_PREWAR)
				{
					return false;
				}
			}

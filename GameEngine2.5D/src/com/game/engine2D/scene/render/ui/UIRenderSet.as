package com.game.engine2D.scene.render.ui
{
	 ...
	 * @author Carver
	 */
	public class UIRenderSet extends UIBaseObj
	{
		//显示和更新控制*********************************************************************************************************
		/**
		 * 是否在可视范围内 
		 ...
		 * 换装部件集合
		 */
		private var _renderUnitList:Vector.<UIRenderUnit> = new Vector.<UIRenderUnit>;
		//-------------------------------------------------------------------------------------------------------------
		/**应用滤镜数组*/
		...
		override public function reSet($parameters:Array):void
		{
			...
			if(!_graphicDis)
			{
				_graphicDis = new starling.display.Sprite();
			}
		}
		...
		private static var _pool:Pool = new Pool("UIRenderSet",1000);
		/**
		 * 生成一个RenderUnit
		 * @param $type
		 * @param $value
		 */
		public static function create():UIRenderSet
		{
			_cnt++;
			//利用池生成
			return _pool.createObj(UIRenderSet) as UIRenderSet;
		}
		
		public static function recycle($av:UIRenderSet):void
		{
			_cnt--;
			//利用池回收
			_pool.disposeObj($av);
		...
		override public function dispose():void
		{
			...
			if (_graphicDis)
			{
				SDisplayUtil.removeAllChild(_graphicDis as DisplayObjectContainer);
		 ...
		 * 注意：不检测回调
		 */
		public function hasRenderUnit($ap:UIRenderUnit,$checkResReady:Boolean=false):Boolean
		{
		 ...
		 * @param $id 换装ID
		 */
		public function getRenderUnitByID($id:int, $checkResReady:Boolean=false):UIRenderUnit
		{
			var ap:UIRenderUnit;
			var len:int = _renderUnitList.length;
			while(len-->0)
		 ...
		 */
		public function addRenderUnit($apd:IRenderUnitData):UIRenderUnit
		{
			//清空同种类型换装
		 ...
		 */
		public function removeRenderUnit($renderUnit:UIRenderUnit,$clearSameType:Boolean=false):void
		{
			//清空同种状态换装
			if($clearSameType)
			{
		 ...
		 * @param $renderUnit
		 */
		public function showRenderUnit($renderUnit:UIRenderUnit):void
		{
		 ...
		 * @param $renderUnit
		 */
		public function hideRenderUnit($renderUnit:UIRenderUnit):void
		{
			$renderUnit.visible = true;
		}
		/**
		...
		public function showRenderUnitByID($id:int):void
		{
			...
			//执行显示
			var ap:UIRenderUnit = getRenderUnitByID($id,false);
			if(ap!=null)
			{
				ap.visible = true;
			}
		 ...
		 */	
		override final public function run(gap:uint):void
		{
			//深度排序
			if(_needSort)
			{
			...
			var numChild:int = Sprite(_graphicDis).numChildren;
			//运行逻辑
			var ap:UIRenderUnit;
			var len:int = _renderUnitList.length;
			var shadowApList:Vector.<UIRenderUnit> = new Vector.<UIRenderUnit>();
			while (len-->0)
			{
				ap = _renderUnitList[len];
				if (ap.usable)
		...
		private function depthSort(apA:UIRenderUnit,apB:UIRenderUnit):int
		{
